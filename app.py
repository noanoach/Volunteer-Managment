...
def index():
    ...
    # Get the activities this user is registered for
    if current_user.is_authenticated:
        registered_activities = VolunteerRegistration.query.filter_by(
            user_id=current_user.id
        ).all()
        registered_activity_ids = [reg.activity_id for reg in registered_activities]
    else:
...
def admin():
    ...
    # Get all activities within the next 7 days
    activities = Activity.query.filter(
        Activity.date >= today,
        Activity.date <= end_date
    ).order_by(Activity.date, Activity.start_time).all()
    
    # Get all future activities for the main list
    all_future_activities = Activity.query.filter(
        Activity.date >= today
    ).order_by(Activity.date, Activity.start_time).all()
    ...
    return render_template('admin.html', 
                         ...
                         all_activities=all_future_activities,
                         today=today,
                         end_date=end_date,
                         timedelta=timedelta)
...
def activity_details(activity_id):
    ...
    activity = Activity.query.get_or_404(activity_id)
    registrations = VolunteerRegistration.query.filter_by(activity_id=activity_id).all()
    
    # Get user details for each registration
    registrations_with_users = []
    for reg in registrations:
        user = User.query.get(reg.user_id)
        registrations_with_users.append({
            'registration': reg,
            'user': user
        })
    ...
    return render_template('activity_details.html',
                         activity=activity,
                         registrations=registrations_with_users)
...
def delete_activity(activity_id):
    ...
    VolunteerRegistration.query.filter_by(activity_id=activity_id).delete()
